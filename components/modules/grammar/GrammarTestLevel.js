...
const GrammarTestLevel = ({level , pastLevel ,name , userStatus , userMark , pastLevelStatus}) => {
    return (
        ...
        </div>
        {
            ...
            userStatus =="passed" ? <Link class="btn btn-linear disabled d-inline-block hover-up hover-shadow mt-3 mt-sm-0 d-flex align-items-center justify-content-center" style={{width:"130px" , height:"40px"}} href={`/tests/grammar/${level}Level`}>شما در این آزمون شرکت کرده اید</Link> :
            userStatus =="failed" && <Link class="btn btn-linear disabled d-inline-block hover-up hover-shadow mt-3 mt-sm-0 d-flex align-items-center justify-content-center" style={{width:"130px" , height:"40px"}} href={`/tests/grammar/${level}Level`}>شما در این آزمون شرکت کرده اید</Link>
        }
        </div>
    </div>
    );
